...
module vt.Graphics.D3D12.DescriptorPool;
...
namespace vt::d3d12
{
	unsigned count_descriptors_to_allocate(ConstSpan<D3D12_DESCRIPTOR_RANGE1> ranges, unsigned dynamic_count)
	{
		unsigned count = 0;
	...
	DescriptorPool::DescriptorPool(ID3D12Device4& device,
								   unsigned		  gpu_view_descriptor_count,
								   unsigned		  gpu_sampler_descriptor_count) :
	...
	SmallList<DescriptorSet> DescriptorPool::make_descriptor_sets(ArrayView<DescriptorSetLayout> set_layouts,
	...
	D3D12_CPU_DESCRIPTOR_HANDLE DescriptorPool::allocate_render_target_views(size_t count)
	{
		return rtv_heap.allocate(count);
	}

	void DescriptorPool::deallocate_render_target_views(D3D12_CPU_DESCRIPTOR_HANDLE rtvs, size_t units)
	{
		if(rtvs.ptr)
			rtv_heap.deallocate(rtvs, units);
	}

	D3D12_CPU_DESCRIPTOR_HANDLE DescriptorPool::allocate_depth_stencil_view()
	{
		return dsv_heap.allocate();
	}

	void DescriptorPool::deallocate_depth_stencil_view(D3D12_CPU_DESCRIPTOR_HANDLE dsv)
	{
		if(dsv.ptr)
			dsv_heap.deallocate(dsv);
	}

	UniqueCpuDescriptor DescriptorPool::allocate_cbv_srv_uav()
	{
		auto handle = cbv_srv_uav_stage_heap.allocate();
		return {handle, {&cbv_srv_uav_stage_heap}};
	}

	UniqueCpuDescriptor DescriptorPool::allocate_sampler()
	{
		auto handle = sampler_stage_heap.allocate();
		return {handle, {&sampler_stage_heap}};
	}

	void DescriptorPool::reset_shader_visible_heaps()
	{
		cbv_srv_uav_gpu_heap.reset();
		sampler_gpu_heap.reset();
	}

	UINT DescriptorPool::get_rtv_stride() const
	{
		return rtv_heap.get_stride();
	}

	UINT DescriptorPool::get_dsv_stride() const
	{
		return dsv_heap.get_stride();
	}

	UINT DescriptorPool::get_cbv_srv_uav_stride() const
	{
		return cbv_srv_uav_stage_heap.get_stride();
	}

	UINT DescriptorPool::get_sampler_stride() const
	{
		return sampler_stage_heap.get_stride();
	}

	D3D12_CPU_DESCRIPTOR_HANDLE DescriptorPool::mirror_view_descriptor(D3D12_GPU_DESCRIPTOR_HANDLE view_descriptor) const
	{
		return cbv_srv_uav_gpu_heap.mirror(view_descriptor);
	}

	D3D12_CPU_DESCRIPTOR_HANDLE DescriptorPool::mirror_sampler_descriptor(D3D12_GPU_DESCRIPTOR_HANDLE sampler_descriptor) const
	{
		return sampler_gpu_heap.mirror(sampler_descriptor);
	}

	// Returns a render target null descriptor for a 2D render target.
	D3D12_CPU_DESCRIPTOR_HANDLE DescriptorPool::get_rtv_null_descriptor() const
	{
	...
	ID3D12DescriptorHeap* DescriptorPool::get_shader_visible_view_heap() const
	{
		return cbv_srv_uav_gpu_heap.get_handle();
	}

	ID3D12DescriptorHeap* DescriptorPool::get_shader_visible_sampler_heap() const
	{
		return sampler_gpu_heap.get_handle();
	}

	void DescriptorPool::initialize_render_target_null_descriptor(ID3D12Device4& device)
	{
	...
	D3D12DescriptorSet DescriptorPool::make_descriptor_set(D3D12DescriptorSetLayout const& layout, unsigned dynamic_count)
	{
	...
	D3D12DescriptorSet DescriptorPool::make_descriptor_set_for_root_descriptor(D3D12DescriptorSetLayout const& layout,
	...
	D3D12DescriptorSet DescriptorPool::make_descriptor_set_for_descriptor_table(D3D12DescriptorSetLayout const& layout,
																				unsigned						dynamic_count)
	{
		auto	 sampler_ranges = layout.get_sampler_descriptor_table_ranges();
		unsigned sampler_count	= count_descriptors_to_allocate(sampler_ranges, dynamic_count);
