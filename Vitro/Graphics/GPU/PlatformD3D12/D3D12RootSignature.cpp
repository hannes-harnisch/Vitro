...
#include <vector>
module vt.Graphics.D3D12.RootSignature;
...
namespace vt::d3d12
{
	void RootParameterMap::emplace(unsigned layout_id, uint8_t view_index, uint8_t sampler_table_index)
	{
		map.try_emplace(layout_id, view_index, sampler_table_index);
	}

	IndexPair RootParameterMap::find(unsigned layout_id) const
	{
		return map.find(layout_id)->second;
	}

	D3D12_ROOT_PARAMETER1 get_push_constant_range(RootSignatureSpecification const& spec)
	{
		return {
	...
	size_t count_ranges(RootSignatureSpecification const& spec)
	{
		size_t count = 0;
	...
	D3D12_ROOT_PARAMETER1 get_descriptor_table_parameter(SmallList<D3D12_DESCRIPTOR_RANGE1>& tracked,
	...
	D3D12RootSignature::D3D12RootSignature(RootSignatureSpecification const& spec, ID3D12Device4& device)
	{
	...
	RootParameterMap const& D3D12RootSignature::get_parameter_map() const
	{
		return parameter_indices;
	}

	ID3D12RootSignature* D3D12RootSignature::get_handle() const
	{
