...
module vt.Graphics.D3D12.Queue;
...
namespace vt::d3d12
{
	...
	Queue::Queue(ID3D12Device4& device, D3D12_COMMAND_LIST_TYPE command_type)
	{
	...
	void Queue::submit(ArrayView<CommandListHandle> cmd_lists, ConstSpan<SyncToken> gpu_wait_tokens) const
	{
	...
	ID3D12Fence* Queue::get_fence() const
	{
		return fence.get();
	}

	ID3D12CommandQueue* Queue::get_handle() const
	{
		return queue.get();
	}
}
