...
module vt.Graphics.D3D12.DescriptorSetLayout;
...
namespace vt::d3d12
{
	...
	D3D12_ROOT_PARAMETER_TYPE determine_view_parameter_type(DescriptorSetLayoutSpecification const& spec)
	{
	...
	size_t count_static_samplers(DescriptorSetLayoutSpecification const& spec)
	{
		size_t count = 0;
	...
	std::atomic_uint D3D12DescriptorSetLayout::id_counter = 0;

	D3D12DescriptorSetLayout::D3D12DescriptorSetLayout(DescriptorSetLayoutSpecification const& spec) :
	...
	unsigned D3D12DescriptorSetLayout::get_id() const
	{
		return id;
	}

	D3D12_SHADER_VISIBILITY D3D12DescriptorSetLayout::get_visibility() const
	{
		return visibility;
	}

	D3D12_ROOT_PARAMETER_TYPE D3D12DescriptorSetLayout::get_view_root_parameter_type() const
	{
		return view_parameter_type;
	}

	ConstSpan<D3D12_STATIC_SAMPLER_DESC> D3D12DescriptorSetLayout::get_static_sampler_descs() const
	{
		return static_samplers;
	}

	bool D3D12DescriptorSetLayout::has_root_descriptor() const
	{
		return view_parameter_type != D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	}

	ConstSpan<D3D12_DESCRIPTOR_RANGE1> D3D12DescriptorSetLayout::get_view_descriptor_table_ranges() const
	{
		return {descriptor_table_ranges.begin(), descriptor_table_ranges.begin() + sampler_range_start_index};
	}

	ConstSpan<D3D12_DESCRIPTOR_RANGE1> D3D12DescriptorSetLayout::get_sampler_descriptor_table_ranges() const
	{
		return {descriptor_table_ranges.begin() + sampler_range_start_index, descriptor_table_ranges.end()};
	}

	size_t D3D12DescriptorSetLayout::count_descriptor_table_ranges() const
	{
		return descriptor_table_ranges.size();
	}

	D3D12_ROOT_PARAMETER1 D3D12DescriptorSetLayout::get_root_descriptor_parameter() const
	{
	...
	unsigned D3D12DescriptorSetLayout::get_range_offset(D3D12_DESCRIPTOR_RANGE_TYPE type, unsigned register_number) const
	{
		return range_offsets.find({type, register_number})->second;
	}

	size_t D3D12DescriptorSetLayout::determine_descriptor_table_range_count(DescriptorSetLayoutSpecification const& spec) const
	{
	...
	void D3D12DescriptorSetLayout::initialize_static_samplers(DescriptorSetLayoutSpecification const& spec)
	{
	...
	void D3D12DescriptorSetLayout::initialize_descriptor_table_ranges(DescriptorSetLayoutSpecification const& spec)
	{
